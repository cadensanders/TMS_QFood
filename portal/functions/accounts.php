		...
		if(isThemeUserRole($role)){
			...
			if($packageOptions['expirationLimit'] != 0){
				...
				if($daysLeft <= 0){
					...
					wp_mail( $user->data->user_email, __("FoodGuide: Account Disabled", 'ait-admin'), "" );	// tu to chce link do admina, pripadne rovno na review accountu
					...
					wp_mail( $user->data->user_email, "FoodGuide: ".$notification['subject'], $notification['message'] );
				} else {
					// nothing here
