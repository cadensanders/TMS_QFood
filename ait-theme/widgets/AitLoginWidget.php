...
class AitLoginWidget extends WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		if( is_user_logged_in() ){
			...
			// register / login
			$noTitle = empty($instance['title']) ? 'no-title' : '';

			$result .= '<div class="userlogin-container user-not-logged-in '.$noTitle.'">';
				$result .= '<div class="userlogin-tabs">';
					...
					if (get_option( 'users_can_register' )) {
						...
						$result .= '<div class="userlogin-tabs-content '.$openedTabLogin.'">';
							...
							$result .= wp_login_form( array( 'form_id' => 'ait-login-form-widget', 'echo' => false, 'remember' => false,) );
						$result .= '</div>';
					$result .= '</div>';
				$result .= '</div>';
				echo($result);
